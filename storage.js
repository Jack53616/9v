...
MysqlStore.prototype.insertUnused = async function (row) {
  await this.pool.query(
    'INSERT INTO licenses (key_hash, key_suffix, plan, status, created_by) VALUES (?, ?, ?, \'unused\', ?)',
    [row.key_hash, row.key_suffix, row.plan, row.created_by]
  );
};
...
async function openStore() {
  if (!process.env.DB_HOST || !process.env.DB_USER || !process.env.DB_NAME) {
    return { kind: 'file', store: new FileStore() };
  }
  ...
  try {
    await pool.query(
      'CREATE TABLE IF NOT EXISTS licenses (' +
        ...
        'plan ENUM(\'minute\',\'weekly\',\'monthly\',\'lifetime\') NOT NULL,' +
    ...
    return { kind: 'mysql', store: new MysqlStore(pool) };
  } catch (_err) {
    try { await pool.end(); } catch (_) {}
    process.stderr.write('mysql unavailable, using file store\n');
    return { kind: 'file', store: new FileStore() };
  }
}
